...
function groupRows(entries) {
  return entries
    .map((entry, i) => ({
      ...
      task: entry.task ?? '',
      // id largo del task (entry.taskNumber): se conserva para trazabilidad/tooltips.
      taskNumber: entry.taskNumber ?? '',
      // código corto de Zoho (entry.taskKey, ej. "PP1-T1"): lo que la grilla MUESTRA junto
      // al nombre con formatTaskLabel (feature task-key-display).
