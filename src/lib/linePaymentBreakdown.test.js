...
test('linePaymentBreakdown: dos parciales → 2 filas, cada una con sus horas, ordenadas por fecha', () => {
  ...
  assert.deepEqual(
    out.map((r) => ({ id: r.id, hours: r.hours, entryCount: r.entryCount })),
    [
      { id: 'pA', hours: 8, entryCount: 2 },
      { id: 'pB', hours: 6, entryCount: 2 },
    ],
  )
})

test('linePaymentBreakdown: sólo cuenta la intersección con la línea (ignora horas de otras líneas)', () => {
  const ic = { entryIds: [1, 2] }
  ...
  assert.equal(out[0].entryCount, 2)
})
...
test('linePaymentBreakdown: sin hoursByEntryId, horas 0 pero el conteo de entries se mantiene', () => {
  const ic = { entryIds: [1, 2] }
